...
class JobController
{
    ...
    {
        $path = directoryRoot('storage/cache/jsons/jobs.json');
    ...
    private function update()
    {
        ...
        if (file_exists($path)) $this->cache = json_decode(file_get_contents($path), true);
    }
    ...
    {
        Shell::exec('php ' . directoryRoot() . ' && php haley job:master > /dev/null 2>&1 &');
    ...
    public function clock()
    {
        ...
        foreach ($this->cache['jobs'] as $key_jobs => $jobs) {
            foreach ($jobs as $key_job => $job) {
                ...
                if (!empty($this->cache['jobs'][$key_jobs][$key_job]['time_limit']) && $this->cache['jobs'][$key_jobs][$key_job]['time_executed'] >= $this->cache['jobs'][$key_jobs][$key_job]['time_limit']) {
                    ...
                    Log::create('jobs', sprintf('TIMEOUT - %s [%s]', $this->cache['jobs'][$key_jobs][$key_job]['name'] ?? '---', $this->cache['jobs'][$key_jobs][$key_job]['description'] ?? '?'));
        ...
        foreach ($this->jobs as $key => $job) {
            ...
            Shell::exec('php ' . directoryRoot() . ' && php haley job:execute ' . $key . ' > /dev/null 2>&1 &', name: $job['name']);
        }
    }
    ...
    public function master()
    {
        ...
        while (true) {
            ...
            Shell::exec('cd ' . directoryRoot() . ' && php haley job:clock >> /dev/null 2>&1');
    ...
    public function execute(int|string $key)
    {
        ...
        if (array_key_exists($key, $this->jobs)) {
            ...
            if ($job['unique']) {
                if (array_key_exists($key, $this->cache['jobs']) && count($this->cache['jobs'][$key])) {
                    return;
                }
            }
